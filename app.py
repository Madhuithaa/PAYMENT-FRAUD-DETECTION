...
clf=['Extreme Gradient Boosting','Random Forest']
mod_feature = st.sidebar.selectbox('Which model for feature importance?', clf)

start_time = timeit.default_timer()
if mod_feature=='Extreme Gradient Boosting':
    model=etree
...
if st.sidebar.checkbox('Run a credit card fraud detection model'):
    ...
    alg=['Extreme Gradient Boosting','Random Forest','k Nearest Neighbor','Support Vector Machine','Logistic Regression']
    ...
    elif classifier == 'Extreme Gradient Boosting':
        model=etree
        if imb_rect=='No Rectifier':
            compute_performance(model, X_train_sfs_scaled, y_train,X_test_sfs_scaled,y_test)
        elif imb_rect=='SMOTE':
